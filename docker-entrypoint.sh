...
PROJECT_ID="${INFISICAL_PROJECT_ID:-648d29ff-c7b9-46dd-926f-cd739808a4df}"
...
# Autentikasi mesin headless menggunakan Universal Auth untuk mendapatkan Identity Access Token
...
if [ -n "$TOKEN" ]; then
    ...
    exec infisical run --token="$TOKEN" --domain="$API_DOMAIN" --env="$ENV_TARGET" $PROJECT_ARG --silent --path=/ -- /app/start.sh "$@"
else
    echo "[Entrypoint] Running start.sh with existing environment..."
    exec /app/start.sh "$@"
fi
