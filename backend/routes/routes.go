...
func Setup(app *fiber.App, db *pgxpool.Pool, cfg *config.Config) {
	...
	sops.Get("/:id", handlers.GetSOP(db, cfg))
	sops.Put("/:id", handlers.UpdateSOP(db, cfg))
	sops.Delete("/:id", handlers.DeleteSOP(db, cfg))
	...
	admin.Get("/user-role", middleware.RateLimit(30, time.Minute), handlers.GetUserRole(db, cfg))
