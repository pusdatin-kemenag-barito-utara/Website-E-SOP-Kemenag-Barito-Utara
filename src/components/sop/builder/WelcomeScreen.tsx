...
export function WelcomeScreen({ onLogin, isLoading }: WelcomeScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoggingIn(true);
    const res = await onLogin(email, password, rememberMe, async () => {
      setShowSuccessAlert(true);
      await new Promise((resolve) => setTimeout(resolve, 2000));
    });

    if (!res?.success) {
      setErrorMsg(res?.error || "Gagal login. Periksa kembali email dan password Anda.");
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center bg-[#F8FAFC] font-sans overflow-hidden select-none p-4 sm:p-6">
      {/* Lightweight Dew Particle Canvas */}
      <DewBackground />
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider ml-0.5">
                Email Admin
              </label>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-center h-11 bg-white rounded-xl border border-slate-200 focus-within:border-[#015C3A] focus-within:ring-1 focus-within:ring-[#015C3A] transition-all duration-200">
                ...
                <button
                  ...
                  className="absolute right-3.5 text-slate-400 hover:text-[#015C3A] transition-colors outline-none"
                >
                  {showPassword ? (
            ...
            <button
              ...
              className="group relative inline-flex h-11 w-full items-center justify-center rounded-xl bg-[#015C3A] hover:bg-[#014a2e] px-8 text-xs font-bold tracking-wider text-white uppercase transition-all focus:outline-none focus:ring-2 focus:ring-[#015C3A] focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-60 cursor-pointer"
            >
