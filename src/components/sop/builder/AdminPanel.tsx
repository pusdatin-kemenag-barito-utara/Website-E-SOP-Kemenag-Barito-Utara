...
const getBidangName = (email: string | undefined | null) => {
  if (!email) return "Lainnya";
  if (email === "baritoutara@kemenag.go.id") return "Proyek Super Admin";
  if (email.startsWith("admin.") && email.endsWith("@kemenag.go.id")) {
    const name = email.split(".")[1].split("@")[0];
    switch (name) {
      case "bimasislam": return "Bimas Islam";
      case "penmad": return "Pendidikan Madrasah";
      case "pdpontren": return "Pondok Pesantren";
      case "pai": return "Admin PAI";
      case "zawa": return "Zakat & Wakaf";
      case "haji": return "Penyelenggaraan Haji & Umrah";
      case "kristen": return "Bimas Kristen";
      case "katolik": return "Bimas Katolik";
      case "hindu": return "Bimas Hindu";
      case "buddha": return "Bimas Buddha";
      case "tu": return "Admin Sub Tata Usaha";
      case "konghucu": return "Bimas Konghucu";
...
export function AdminPanel({
  ...
  // Reset state when modal opens using the recommended render-phase approach
  ...
  const groupedSops = useMemo(() => {
    ...
    allSops.forEach(sop => {
      const bidang = getBidangName(sop.user_email);
      if (!groups[bidang]) groups[bidang] = [];
      groups[bidang].push(sop);
    });
    
    // Sort to make "Proyek Super Admin" first
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-950 w-full max-w-5xl h-[85vh] rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] ring-1 ring-black/5 flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        {/* Header */}
        <div className="p-6 md:p-8 bg-gradient-to-r from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 text-white flex items-center justify-between relative overflow-hidden transition-all duration-500">
          ...
          <div className="flex items-center gap-4 relative z-10">
            {selectedBidang ? (
              <Button
                variant="ghost"
                size="icon"
                className="text-white hover:bg-white/10 rounded-full h-10 w-10 shrink-0 mr-1"
                onClick={() => {
                  setSelectedBidang(null);
                  setSearch("");
                }}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="text-slate-300 hover:text-white hover:bg-white/10 rounded-xl h-10 w-10 relative z-10 transition-colors shrink-0"
            onClick={onClose}
            aria-label="Tutup"
          >
            <X className="w-5 h-5" />
        ...
        {/* Search & Stats */}
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-800 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm w-full md:w-auto justify-center md:justify-start">
            ...
            <span className="font-semibold">{selectedBidang ? "Total Dokumen" : "Total Bidang"}</span>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="p-3.5 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 text-center relative z-10">
